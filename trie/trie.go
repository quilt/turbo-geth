...
func (t *Trie) GetAccount(key []byte) (value *accounts.Account, gotValue bool) {
	...
	if accNode != nil {
		...
		t.observers.AccountTouched(hex)
...
func (t *Trie) getAccount(origNode node, key []byte, pos int) (value *accountNode, gotValue bool) {
	...
	case *accountNode:
		t.observers.AccountTouched(key)
		return n, true
	default:
...
func (t *Trie) touchAll(n node, hex []byte, del bool) {
	...
	case *duoNode:
		if !del {
			t.observers.StructNodeLoaded(hex)
		}
		i1, i2 := n.childrenIdx()
...
func (t *Trie) unloadMany(hexes []string) {
	//hex := []byte(hexStr)
	//t.unload(hex)
}

func (t *Trie) TrieSize() int64 {
	return sizeOfAccounts(t.root)
}

func (t *Trie) NumberOfAccounts() int64 {
	return numberOfAccounts(t.root)
}
